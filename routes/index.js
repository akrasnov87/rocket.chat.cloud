...
router.get(
  "/api/v2/workspaces/:device_code/license",
...
router.post(
  "/api/v2/workspaces/:device_code/client/downgrade",
  function (req, res, next) {
    console.get(
      `POST /api/v2/workspaces/:device_code/client/downgrade - is empty`
    );
...
router.post(
  "/api/v2/workspaces/:device_code/client",
  function (req, res, next) {
    res.json({
      workspaceId: process.env.WORKSPACE_ID,
      publicKey:
...
router.get("/api/v2/register/workspace/poll", function (req, res, next) {
  res.json({
    ...
    payload: {
      ...
      redirect_uris: [
        `${req.protocol}://${req.headers["host"]}/admin/cloud/oauth-callback`,
      ],
      publicKey:
      ...
      registration_client_uri: `${req.protocol}://${req.headers["host"]}/api/v2/workspaces/${process.env.DEVICE_CODE}`,
      licenseData: {
        version: 0,
        address: "",
        license: "",
...
router.post("/api/oauth/clients", function (req, res, next) {
  res.json({
    ...
    publicKey:
      "-----BEGIN RSA PUBLIC KEY-----\\nMIIBCgKCAQEAv54YJeF7sFWCQmjJn7cZmvKa3i9W4eKGW0E6T3z5hFyFi1lNMQ/S\\nzXlleLGa0/OM4jgW47dfuMZ5TSTvq1AyA0PUoHDpOzSrmGWBTYCQXgVOXhZC0LKX\\n6IC86RhFmHJ8Rr5GAI6VDWNuBiWPVGDFC17/XzdERUSv16CkaMbbCfRI8RF0R5d+\\nOZq4aD1p7pxlEEjkAQbtpPfRfs9VVAuKq6pBroJkAxkFCa8k3zdl2sKlqNuGerBX\\nfzMlYCJS+4LNOEdb70FqHHxDA8wybJMcYoWA+Fd3W1sRejeGBZJPtSSQ0Jw1BjCx\\nPedMK5iMG9ZjMQLxcNdf0fc21oKJDq7V7QIDAQAB\\n-----END RSA PUBLIC KEY-----\\n",
    client_secret_expires_at: 0,
    registration_client_uri: `${req.protocol}://${req.headers["host"]}/api/v2/workspaces/${process.env.DEVICE_CODE}`,
    licenseData: {
      version: 0,
...
router.post("/api/oauth/token", function (req, res, next) {
  res.json({
    access_token:
      "/bz6cAGjTeeIRtIJzCw/CIvGJZyqIhKZ474GbKH0pFwfU2QAAA3QhNc2elm1VQt3M24hUybJzK/K9eS55V1hn4JifWm6i5yHe1PB12K8BQc=",
    expires_in: 7200,
    scope:
      "workspace:license:read workspace:client:write workspace:stats:write workspace:push:send marketplace:read marketplace:download fedhub:register",
    token_type: "Bearer",
  });
});
